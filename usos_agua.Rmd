...
datos_l <- datos %>%
           select(-"Regional",-"Estado",-"Tipo Solicitud",-"Fecha Solicitud",-"Fecha Resolución",-"Fecha Inscripción",
                  -"Fecha Vencimiento",-"produccion",-"Inicio",-"Fin") %>%
...
arbol_1 <- rpart(formula = uso ~ dpto + vol_anual + tipo_ext, data = training)
```
```{r}
rpart.plot(arbol_1)
```

En el modelo anteriormente desarrollado podemos ver, antes de realizar la predicción con el conjunto de testing, que el modelo no está prediciendo la ocurrencia de "Consumo humano", "Otros usos" y "Usos no consuntivos"
...
```{r}
arbol_2 <- rpart(formula = uso ~ dpto + vol_anual + tipo_ext, data = training, weight = Proporcion)
...
datos_sin_na <- datos_l %>%
  filter(!is.na(dpto) & !is.na(vol_anual) & !is.na(tipo_ext))
...
rf_1 <- randomForest(uso ~ dpto + vol_anual + tipo_ext, data= training_rf, ntree = 150)
...
```{r}
rf_nc_1 <- randomForest(uso ~ dpto + vol_anual + tipo_ext, data= training_nc, ntree = 10)
