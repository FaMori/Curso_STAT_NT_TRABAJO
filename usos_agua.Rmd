...
## Cuencas hidrograficas catalogadas por destino mayoritario y volumen anual disponible
...
datos_mod <- datos_l %>%
             select(uso,vol_anual,mes_anio,tipo_ext,codcuenca,dias_permiso,codcuenca,dpto) %>%
...
arbol_1 <- rpart(formula = uso ~ dpto + vol_anual + tipo_ext + dias_permiso, data = datos_training)
```
...
```{r}
arbol_2 <- rpart(formula = uso ~ dpto + vol_anual + tipo_ext + dias_permiso, data = datos_arbol_2, weight = proporcion)
...
```{r}
rf_1 <- randomForest(uso ~ codcuenca + vol_anual + tipo_ext + dias_permiso, data= datos_training, ntree = 150)
